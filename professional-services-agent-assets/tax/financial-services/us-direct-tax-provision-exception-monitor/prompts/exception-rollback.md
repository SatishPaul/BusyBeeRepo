...
2. Take a version snapshot of the provision run before any change.
3. If the rejection invalidates downstream records, list them and pause; do not cascade automatically.
4. On my explicit confirmation, roll the run back to the snapshot and log a rollback event.
