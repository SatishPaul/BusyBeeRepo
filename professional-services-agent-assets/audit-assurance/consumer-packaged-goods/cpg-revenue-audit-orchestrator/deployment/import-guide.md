...
1. Download `dataverse/psacRevenueAuditOrchestrator_unmanaged.zip` from the repo.
...
5. Open the model-driven app and confirm the FY26-Q4 screen shows 40 transactions and 3 findings.
...
3. Trigger a run. The master creates a revenue screen, the specialists test the transactions, findings appear, and an approval is opened.
4. Work the exceptions in the approval inbox; approve or reject each; roll back if needed.

## Verify

- The screen shows the correct exception count and confidence.
- Every finding cites a transaction and a confidence score.
- No finding was written for an in-band, delivered transaction (no false positives).
- The approval gate blocked any consequential action until a human decided.

## Known limitations and preview dependencies

Microsoft Scout is a preview product; its scheduling, delegation, and export behavior can change, so the Scout portions may need adjustment as the product evolves. The ROI figures are sample assumptions until measured. The agents test and explain; they do not post adjustments or sign the opinion, by design. Use synthetic data only in any shared or public copy of this pack.
