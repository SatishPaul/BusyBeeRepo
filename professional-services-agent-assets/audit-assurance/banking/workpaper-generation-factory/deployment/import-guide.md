...
- Microsoft 365 Copilot and Copilot Studio access, plus Copilot Cowork for the source assembly.
...
1. Download `dataverse/pswpWorkpaperFactory_unmanaged.zip` from the repo.
...
5. Open the model-driven app and confirm the FY26 workpaper shows 18 sections and 3 findings.
...
4. Wire the Cowork assembly with Prompt 2, and the Scout schedule to invoke the master agent on your fieldwork cadence.

## Configure and run

1. Set the environment variables in `dataverse/env-variables.md` (confidence threshold, tie-out tolerance, coverage band, approver).
2. Apply the security roles in `dataverse/security-roles.md` (least privilege, read-only defaults).
3. Trigger a run. Cowork assembles the source file, the master creates a workpaper, the specialists draft and tie out the sections, findings appear, and an approval is opened.
4. Work the flagged sections in the approval inbox; approve or reject each; roll back if needed.

## Verify

- The workpaper shows the correct flagged-section count and confidence.
- Every finding cites a section and a confidence score.
- No finding was written for a tied, in-band, complete section (no false positives).
- The approval gate blocked any consequential action until a human decided.

## Known limitations and preview dependencies

Microsoft Scout is a preview product; its scheduling, delegation, and export behavior can change, so the Scout portions may need adjustment as the product evolves. Copilot Cowork behavior can also change during preview. The ROI figures are sample assumptions until measured. The agents draft and explain; they do not sign the workpaper or conclude the opinion, by design. Use synthetic data only in any shared or public copy of this pack.
