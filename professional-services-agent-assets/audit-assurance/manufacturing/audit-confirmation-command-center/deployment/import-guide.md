...
1. Download `dataverse/psacConfirmationCommandCenter_unmanaged.zip` from the repo.
...
5. Open the model-driven app and confirm the FY26 cycle shows 40 confirmations and 3 findings.
...
4. Wire the Scout schedule to invoke the master agent on your fieldwork cadence.

## Configure and run

1. Set the environment variables in `dataverse/env-variables.md` (confidence threshold, reconciliation tolerance, due-date window, approver).
2. Apply the security roles in `dataverse/security-roles.md` (least privilege, read-only defaults).
3. Trigger a run. The master creates a confirmation cycle, the specialists track and reconcile the confirmations, findings appear, and an approval is opened.
4. Work the exceptions in the approval inbox; approve or reject each; roll back if needed.

## Verify

- The cycle shows the correct exception count and confidence.
- Every finding cites a confirmation and a confidence score.
- No finding was written for a matched, in-tolerance, signed confirmation (no false positives).
- The approval gate blocked any consequential action until a human decided.

## Known limitations and preview dependencies

Microsoft Scout is a preview product; its scheduling, delegation, and export behavior can change, so the Scout portions may need adjustment as the product evolves. The ROI figures are sample assumptions until measured. The agents track and explain; they do not waive confirmations or sign the opinion, by design. Use synthetic data only in any shared or public copy of this pack.
